    ...
    def prepare_data(self):
        ...
        if not osp.exists(done_flag_path):  # Will take ~3 hours...
            ...
            if self.commit == 'sgc_rgat':
                paper_feat = np.load(f'{dataset.dir}/paper_feat_{self.commit}.npy', mmap_mode='r')
            else:
...
class RGNN(LightningModule):
    def __init__(self, commit: str, model: str, in_channels: int, out_channels: int,
    ...
    def forward(self, x: Tensor, adjs_t: List[SparseTensor], save_embed=False) -> Tensor:
...
if __name__ == '__main__':
    ...
    if args.evaluate:
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                out = model(batch.x, batch.adjs_t, args.save_embed).cpu()
                y_preds.append(out)
        y_preds = torch.cat(y_preds, dim=0)
        y_trues = datamodule.y[datamodule.test_idx]
        ...
        for batch in tqdm(loader):
            ...
            with torch.no_grad():
                ...
                y_preds.append(out)
        y_preds = torch.cat(y_preds, dim=0)
        y_trues = datamodule.y[datamodule.val_idx]
